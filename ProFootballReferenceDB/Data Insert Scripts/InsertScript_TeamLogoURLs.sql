IF OBJECT_ID('tempdb..#TeamLogos','U') IS NOT NULL
	DROP TABLE #TeamLogos

CREATE TABLE #TeamLogos (TeamName VARCHAR(75), LogoURL VARCHAR(MAX))

INSERT INTO #TeamLogos (TeamName, LogoURL)
SELECT TeamName, LogoURL FROM (VALUES 
...
,	('Cincinnati Bengals','CIN','https://res.cloudinary.com/nflleague/image/private/f_auto/league/okxpteoliyayufypqalq')
,	('Cleveland Browns','CLE','https://static.www.nfl.com/image/private/f_auto/league/fgbn8acp4opvyxk13dcy')
...
,	('Los Angeles Rams','LA','https://static.www.nfl.com/image/private/f_auto/league/ayvwcmluj2ohkdlbiegi')
...
,	('Washington Commanders','WAS','https://static.www.nfl.com/image/private/f_auto/league/xymxwrxtyj9fhaemhdyd')
)
 AS A (TeamName, TeamAbbrev, LogoURL)


INSERT INTO TeamLogos (TeamName, LogoURL)
SELECT TeamName, LogoURL
FROM #TeamLogos tl
WHERE NOT EXISTS (SELECT 1 FROM TeamLogos tl2 WHERE tl.TeamName = tl2.TeamName)